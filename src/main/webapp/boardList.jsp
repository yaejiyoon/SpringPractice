...
<body>
	<table border=2 align=center>
					...
					<c:forEach var="tmp" items="${result}">
						<tr align=center>
							...
							<td width=150 align=center><a
								href="article.do?seq=${tmp.seq}">${tmp.title}</td>
							<td>${tmp.writer}</td>
							<td>${tmp.writedate}</td>
							<td>${tmp.viewcount}</td>
						</tr>
		...
		<tr>
			<th colspan='4'><input id="searchText" type=text
				placeholder="제목을 검색해주세요" name="searchTitle">
			</td>
			<td><button type="button" id="searchBt">search</button></td>
		</tr>
		<tr>
			<td colspan="6" align=right>
				<button type=button id=write onclick="location.href='write.do'">글쓰기</button>
			</td>
		</tr>
